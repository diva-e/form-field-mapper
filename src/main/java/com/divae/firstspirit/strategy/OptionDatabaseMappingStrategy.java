...
public class OptionDatabaseMappingStrategy implements MappingStrategy {
    ...
    public <O> void map(final AnnotatedMember from, final O fromObject, final FormField<?> to, final GomFormElement toGomFormElement, final Language language, final SpecialistsBroker specialistsBroker) {
        ...
        final Dataset dataset = map(from.get(fromObject), language, GOM_FORM_ELEMENT_SERVANT.getTableTemplate(toGomFormElement, specialistsBroker, language));
        final OptionFactory optionFactory = ((OptionFactoryProvider) toGomFormElement).getOptionFactory();

        to.set(optionFactory.create(dataset.getEntity()));
    }
    ...
    <O> Dataset map(final O object, final Language language, final TableTemplate tableTemplate) {
        final Class<?> objectClass = object.getClass();
        final String databaseUid = DATABASE_SERVANT.getDatabaseUid(objectClass);
        if (databaseUid == null) {
            return null;
        ...
        return DATASET_SERVANT.findDataset(databaseUid, DATABASE_SERVANT.getColumnValueMapping(objectClass, object), tableTemplate, language);
    }
}