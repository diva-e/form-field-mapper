...
public final class FormDataListServant {
    ...
    Entity findEntity(final Schema schema, final Mapping[] tableTemplateMappings, final Map<String, Object> columnValueMapping, final Language language) {
        ...
        final Stream<Mapping> tableTemplateMappingsStream = Stream.of(tableTemplateMappings);
        ...
        columnValueMapping.entrySet().parallelStream().forEach(columnValue -> {
            final Optional<String> attributeNameOptional = tableTemplateMappingsStream.parallel()
                    .filter(tableTemplateMapping -> Stream.of(tableTemplateMapping.getAttributeReference(languageAbbreviation)).parallel().anyMatch(attributeReference -> attributeReference.equals(columnValue.getKey())))
